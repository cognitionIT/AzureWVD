...
.NOTES
    ...
    Updated:        2020-06-28
                    Added new functions that work with the generic ID input
    Updated:        2020-07-22
...
#region PowerShell Modules Functions
...
#endregion PowerShell Modules Functions
...
function Connect-WVDServicePrincipalAccount () {
    ...
    #debug: $SPCredentials.UserName
    #debug: $SPCredentials.GetNetworkCredential().Password
...
function Get-WVDHostPoolUser () {
    ...
    # Check if the UPN parameter is empty or not
    If ([string]::IsNullOrEmpty($UPN))
    {
        $UPN = ""
    }

    # Retrieve the User information for the given Subscription ID (and optional HostPoolName)
    try
    {
        #Retrieve the WVD Host Pool information for this Subscription
        [array]$wvdUserCollection = Get-AzWvdHostPool -SubscriptionId $SubscriptionID -WarningAction SilentlyContinue | 
        foreach {
            $wvdHostPool = $_
            If ($wvdHostPool.Name -like "$HostPoolName*")
            {
                ...
                foreach {
                    $wvdUserSession = $_
                    If ($wvdUserSession.UserPrincipalName -like "$UPN*")
                    {
                        # retrieve User Sessions per HostPool
    ...
    return $wvdUserCollection
}
#endregion Host Pool Functions

#region Session Host Functions
...
function Stop-WVDSessionHost () {
    ...
    # Retrieve the SessionHost
    try
    {
        #Retrieve the Az VM information for this WVD SessionHost
        Get-AzVM -Name $($SessionHostName.Split(".")[0]) | Stop-AzVM -Force
    }
    catch
    {
        Write-Error ("A [" + $_.Exception.GetType().FullName + "] ERROR occurred. " + $_.Exception.Message)
...
function Start-WVDSessionHost () {
    ...
    # Retrieve the VM, based on WVD SessionHost Name
    try
    {
        #Start the Az VM, based on the WVD SessionHost name
        Get-AzVM -Name $($SessionHostName.Split(".")[0]) | Start-AzVM
    }
    catch
    {
        Write-Error ("A [" + $_.Exception.GetType().FullName + "] ERROR occurred. " + $_.Exception.Message)
...
#endregion Session Host Functions
