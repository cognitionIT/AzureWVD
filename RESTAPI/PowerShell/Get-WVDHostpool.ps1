<#
.SYNOPSIS
    Get WVD Host Pool information, sorted by Name.
.DESCRIPTION
    Get WVD Host Pool information, using REST API calls.
.EXAMPLE
    Get-WVDHostPool -HostPoolName <string>
.CONTEXT
...
If ($azSPCredentials = Get-AzSPStoredCredentials)
{
    ...
    # Retrieve the WVD Hostpool details
    $hostPoolResults = Get-WVDHostPool -bearerToken $($azBearerToken.access_token) -SubscriptionID $($azSubscription.subscriptionId) -HostPoolName $HostpoolName

    # Present the information
    foreach ($hostpool in $hostPoolResults)
    {
        Write-host "Host Pool $($hostpool.name) details: " -ForegroundColor Yellow
        $hostpool | Select @{Name='ID'; Expression={$_.id}}, 
            ...
            @{Name='Preferred AppGroup Type'; Expression={$_.properties.preferredAppGroupType}} | Format-List
    }
}
else 
{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUbP1p00xTqEee+FxA47skFePf
# irCgggpeMIIFJjCCBA6gAwIBAgIQCyXBE0rAWScxh3bGfykLTjANBgkqhkiG9w0B
...
# AQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBT9PMD9k4W4
# GXcsbG152YN1L94VcDANBgkqhkiG9w0BAQEFAASCAQCzh9exzAfIxdeNhmVFpCpd
# pQpz8lBoLTVPBFnCL8dnDMnFP0pedSd18/BaNrpmu4cneHW5owj4+hK5mmhfTmLg
# jo0WCL+gKSQK096/TMD83eKWAvgy/V8YzPA73K3G0tdVqgXdmRUk5KAiOLNwGNoU
# QvtzFfoefmg5wDTlIzBWlEgw7UEUpF831mwOmgiWnxQUCPTcDtzNXwXElp+MI9fQ
# 98K0BLhlTHYx0ommkPVNREA0Unki0PxxtSc4ObPcNYRfGHsoEfIOv3ca/S6TZLFb
# BHrA1O4pTQZcVgnvkrp7KFO1xFG9IATQ6Df5UBto3exN+cmPuuzCkSjslsEcdXf8
